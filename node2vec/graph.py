...
class Graph:
    def __init__(self, graph, directed, p, q, num_walks, walk_length):
        self.graph = graph
        ...
        for edge in graph.edges():
            ...
            if not directed:
                self.alias_edges[(edge[1], edge[0])] = self.alias_edge(edge[1], edge[0])
    ...
    def alias_edge(self, pre, cur):
        ...
        for neighbor in neighbors:
            if neighbor == pre:
                alpha = 1. / self.p
            elif self.graph.has_edge(neighbor, pre):
                alpha = 1.
            else:
                alpha = 1. / self.q
