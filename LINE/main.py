...
for i in range(2):
    print('For round %d' % (i + 1), flush=True)
    print(' >> Initializing node2vec graph.', flush=True)
    graph = Graph(nx_graph, args.directed, p[i], q[i], args.num_walks, args.walk_length)
...
with open(node_emb_file, 'w') as f:
    line = [str(len(graph_nodes)) + ' ' + str(args.embed_dim) + '\n']
    for node in graph_nodes:
        tmp = [node] + list(node_models[0][str(node)]) + list(node_models[1][str(node)])
        line.append(' '.join([str(t) for t in tmp]) + '\n')
    f.writelines(line)
