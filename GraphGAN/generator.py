...
class Generator(object):
    def __init__(self, n_node, embed_init):
        self.n_node = n_node
        self.embed_init = embed_init

        with tf.variable_scope('Generator'):
            self.node_embed =  tf.get_variable(name="node_embed", shape=self.embed_init.shape,
                                               initializer=tf.constant_initializer(self.embed_init), trainable=True)
